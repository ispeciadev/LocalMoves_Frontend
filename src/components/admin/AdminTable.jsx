...
const AdminTable = ({ title, columns, rows, actionLabel }) => {
  ...
  return (
    <div className={`mt-5 rounded-2xl p-4 shadow-sm transition-colors ${
      isDarkMode
      ...
      <div className="overflow-x-auto">
        <table className="min-w-full text-left text-xs">
          <thead>
            <tr className={`border-b text-[11px] uppercase font-medium transition-colors ${
              isDarkMode
                      ...
                      <td key={col.key} className="px-3 py-2">
                        <span
                          className={`rounded-full px-2.5 py-0.5 text-[11px] transition-colors ${
                            statusColors[value]?.(isDarkMode) || (isDarkMode ? "bg-slate-700 text-slate-300" : "bg-gray-100 text-gray-700")
                          }`}
                        >
                          {value}
                        </span>
