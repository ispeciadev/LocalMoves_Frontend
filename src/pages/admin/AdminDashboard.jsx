...
function StatCard({ label, value, icon: Icon, highlight, isDarkMode }) {
  ...
  return (
    <div className={`backdrop-blur-md border shadow-sm hover:shadow-md rounded-2xl p-4 md:p-5 flex items-center justify-between transition-all duration-200 ${
      isDarkMode
        ? "bg-slate-800/90 border-slate-700"
        : "bg-white/90 border-pink-200"
    }`}>
      <div>
        <p className={`text-[11px] md:text-xs font-medium uppercase tracking-wide transition-colors ${
          isDarkMode ? "text-slate-400" : "text-gray-400"
        }`}>
          {label}
        </p>
        <p className={`mt-2 text-2xl md:text-3xl font-semibold transition-colors ${
      ...
      <div
        className={`h-10 w-10 md:h-12 md:w-12 rounded-2xl flex items-center justify-center shadow-sm ${
          highlight
...
function MetricCard({ title, value, badge, isDarkMode }) {
  const numeric = Number(value ?? 0);
  return (
    <div className={`backdrop-blur-md border hover:shadow-md shadow-sm rounded-2xl p-4 md:p-5 transition-all duration-200 ${
      isDarkMode
      ...
      <div className="flex items-center justify-between mb-2">
        ...
        {badge && (
          <span className={`inline-flex items-center rounded-full px-3 py-0.5 text-[11px] font-medium shadow-sm transition-colors ${
            isDarkMode
      ...
      <p className={`text-2xl md:text-3xl font-semibold transition-colors ${
        isDarkMode ? "text-slate-100" : "text-gray-800"
      }`}>
        ₹{numeric.toLocaleString("en-IN")}
      </p>
...
function RangeTabs({ value, onChange, isDarkMode }) {
  return (
    <div className={`inline-flex rounded-full px-1.5 py-1 text-[11px] md:text-xs shadow-inner transition-colors ${
      isDarkMode
        ? "bg-slate-700"
        : "bg-gray-100"
    }`}>
      {[
        ...
        { key: "one_month", label: "30 Days" },
        { key: "one_year", label: "12 Months" },
      ].map((opt) => (
        <button
          key={opt.key}
          type="button"
          onClick={() => onChange(opt.key)}
          className={`px-2.5 md:px-3 py-1 rounded-full transition-all font-medium ${
            value === opt.key
...
function RevenueTooltip({ active, payload, label, isDarkMode }) {
  ...
  return (
    <div className={`rounded-xl border px-3 py-2 text-xs shadow-md transition-colors ${
      isDarkMode
      ...
      <p className="mt-1 text-pink-400 font-medium">
...
function UsersTooltip({ active, payload, label, isDarkMode }) {
  ...
  return (
    <div className={`rounded-xl border px-3 py-2 text-xs shadow-md transition-colors ${
      ...
      <p className="mt-1 text-fuchsia-400 font-medium">
...
function AdminDashboard() {
  ...
  const [selectedRevenueRange, setSelectedRevenueRange] =
    useState("seven_days");
  ...
  const fetchDashboard = async () => {
    try {
      ...
      const [statsRes, revenueRes, userRes] = await Promise.all([
  ...
  const revenueChartData = revenueRangeData.map((item) => ({
    rawLabel:
      selectedRevenueRange === "one_year" ? item.month : item.date,
    label:
  ...
  return (
    <div className="space-y-6 pb-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-3">
        <div>
          <h2 className={`text-xl md:text-2xl font-semibold transition-colors ${
            isDarkMode ? "text-slate-100" : "text-gray-800"
          }`}>
            Admin Dashboard
          </h2>
        ...
        <button
          ...
          className={`self-start md:self-auto px-4 py-2 rounded-full border text-xs md:text-sm hover:transition flex items-center gap-2 transition-colors ${
            isDarkMode
      ...
      {loading && (
        <div className="w-full flex justify-center py-20">
          <div className="flex flex-col items-center gap-3">
            <div className={`h-8 w-8 border-2 border-t-transparent rounded-full animate-spin transition-colors ${
      ...
      {/* ERROR */}
      {!loading && error && (
        <div className={`p-4 border rounded-2xl text-sm flex items-start justify-between gap-3 transition-colors ${
          isDarkMode
      ...
      {!loading && !error && (
        <>
          ...
          <div className="grid grid-cols-1 xl:grid-cols-2 gap-6 mt-2">
            {/* Revenue Chart */}
            <div className={`backdrop-blur-md border shadow-md rounded-2xl p-4 md:p-5 flex flex-col gap-4 transition-colors ${
              isDarkMode
                ? "bg-slate-800/90 border-slate-700"
                : "bg-white/90 border-pink-200"
            }`}>
              ...
              <div className="h-64 w-full">
                {revenueChartData.length === 0 ? (
                  <div className={`h-full flex items-center justify-center text-xs transition-colors ${
              ...
              <div className={`mt-1 flex items-center justify-between text-[11px] md:text-xs border-t pt-3 transition-colors ${
                isDarkMode
                  ...
                  <ResponsiveContainer width="100%" height="100%">
                    ...
                    >
                      ...
                      <YAxis
                        ...
                        axisLine={{ stroke: isDarkMode ? "#475569" : "#e5e7eb" }}
                      />
                      <Tooltip content={<UsersTooltip isDarkMode={isDarkMode} />} />
                      <Bar
                        dataKey="count"
                        radius={[6, 6, 0, 0]}
                        maxBarSize={40}
          ...
          <div className={`backdrop-blur-md border shadow-md rounded-2xl p-4 md:p-6 mt-4 transition-colors ${
            isDarkMode
              ? "bg-slate-800/90 border-slate-700"
              : "bg-white/90 border-pink-200"
          }`}>
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 mb-4">
              ...
              {totalSubscriptions > 0 && (
                <span className={`text-[11px] md:text-xs transition-colors ${
                  isDarkMode ? "text-slate-400" : "text-gray-500"
                }`}>
                  Total subscriptions:{" "}
                  <span className={`font-medium transition-colors ${
              ...
              {subscriptions.map((sub) => {
                ...
                return (
                  <div
                    ...
                    className={`backdrop-blur border rounded-2xl p-4 shadow-sm hover:shadow-md transition-all ${
                      isDarkMode
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mt-6">
            {/* Recent Users */}
            <div className={`backdrop-blur-md border shadow-md rounded-2xl p-4 md:p-5 transition-colors ${
              isDarkMode
                ? "bg-slate-800/90 border-slate-700"
                : "bg-white/90 border-pink-200"
            }`}>
              ...
              <div className={`overflow-x-auto scroll-thin-pink rounded-xl border transition-colors ${
                isDarkMode ? "border-slate-700" : "border-gray-100"
              }`}>
                <table className="min-w-[560px] text-xs md:text-sm">
                        ...
                        className={`border-t transition-colors ${
                          idx % 2 === 1
                            ? isDarkMode
                              ...
                              : "bg-gray-50/60"
                            : ""
                        } ${isDarkMode ? "hover:bg-slate-700/50" : "hover:bg-pink-50/80"}`}
                        style={{ borderColor: isDarkMode ? "#475569" : "#e5e7eb" }}
                      >
                        <td className={`py-2 px-3 font-medium transition-colors ${
                          isDarkMode ? "text-slate-100" : "text-gray-900"
                        }`}>
                          {u.full_name || "-"}
                        </td>
                        <td className={`py-2 px-3 hidden sm:table-cell transition-colors ${
                          isDarkMode ? "text-slate-300" : "text-gray-700"
                        }`}>
                          {u.email || "-"}
                        </td>
                        <td className="py-2 px-3">
                          <span className={`inline-flex items-center rounded-full px-2 py-0.5 text-[10px] font-medium transition-colors ${
                            isDarkMode
                              ? "bg-pink-900/30 text-pink-300"
                              : "bg-pink-100 text-pink-800"
                          }`}>
                        ...
                        <td className={`py-2 px-3 hidden md:table-cell transition-colors ${
                          isDarkMode ? "text-slate-400" : "text-gray-500"
                        }`}>
                    ...
                    {(recent.payments || []).slice(0, 10).map((p, idx) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <td className={`py-2 px-3 font-semibold transition-colors ${
                            isDarkMode ? "text-slate-100" : "text-gray-900"
                          }`}>
                  ...
                  <tbody>
                    {(recent.requests || []).slice(0, 10).map((r, idx) => (
                      ...
                      >
                        ...
                        <td className={`py-2 px-3 transition-colors ${
